...
// @TODO Make possible to select another platform at build time.
    ...
    function typeofbtn(t) {
        ...
        switch (t) {
            ...
            case 'farei':
                dt.farei = 1;
                break;
            case 'positivenewlester':
                onNewlestter();
                break;
            case 'negativenewlester':
                sendData();
                break;
            default:
                s = false;
    ...
    //ativa o formulario
    function onNewlestter() {
        $('#anwserquestion').css('display', 'none');
        $('#formnewlester').css('display', 'block');
    }

    //Valida o formulário e aloca as informações
    function validForm() {
        dt.nome = $('#nomenl').val();
        dt.email = $('#emailnl').val();
        dt.sexo = $('#sexonl').val();
        dt.idade = $('#idadenl').val();
        if (dt.nome == '' || dt.email == '' || dt.sexo == '' || dt.idade == '') {
            $('#msgform').html("Algum campo não foi preenchido. Por favor preencha.");
        } else {
            $('#msgform').html("Aguarde...");
            sendData();
        }
    }
    ...
    //verifica os botões de estatisticas
    $('.btnquestion').on('click', function () {
        if (!tbutton()) {
            typeofbtn($(this).attr('data-type'));
            recoveryAnime($(this).attr('data-id'), $(this).attr('data-src'));
            pushslide();
        }
    });

    //verifica se deseja receber informações
    $('.btnquestnewlest').on('click', function () {
        if (!tbutton()) {
            typeofbtn($(this).attr('id'));
        }
    });

    //"envia" o formulário
    $('#sendformnewlester').on('submit', function (e) {
        if (!tbutton()) {
            e.preventDefault();
            validForm();
        }
    ...
    //Avança os slides
    //As IDs carregadas em var são contadas de 0 a 3, A ultima ID é chamada a parte
    var slide = ['questionone', 'questiontwo', 'newlester', 'thanksforfeedback', 'graphicstatistics'];
    //Agulha de posição
    var needle = 0;
    //Função avançar
    function pushslide() {
        setTimeout(function () {
            $('#' + slide[needle]).removeAttr('class', 'slideOpen');
            $('#' + slide[needle]).attr('class', 'slideOff');
            needle++;
            if (needle >= slide.length) {
                needle = 0;
            }
            $('#' + slide[needle]).removeAttr('class', 'slideOff');
            $('#' + slide[needle]).attr('class', 'slideOpen');
        }, 1500);
    }

    //Fecha todos os slides
    function clearSlides() {
        var i;
        var valI = slide.length;
        for (i = 0; i <= valI; i++) {
            $('#' + slide[needle]).removeAttr('class');
            $('#' + slide[needle]).attr('class', 'slideOff');
        }
        needle = 0;
    }

    //Abre um slide em especifico
    function getSlide(p) {
        needle = p;
        $('#' + slide[needle]).removeAttr('class', 'slideOff');
        $('#' + slide[needle]).attr('class', 'slideOpen');
    ...
    function graphicTwo(n, t, s) {
        new Chart(document.getElementById("canvas2"), {
            "type": "doughnut",
            "data": {
                "labels": ["Não farão", "Talvez", "Farão"],
                "datasets": [{
    ...
    function recoveryAnime(id, newSrc) {
        var src = $('#' + id).attr('src');
        $('#' + id).attr('src', newSrc);
        setTimeout(function () {
            $('#' + id).attr('src', src);
    ...
    /*$('.btnquestion').hover(function () {
        if (!tbutton()) {
            recoveryAnime($(this).attr('data-id'), $(this).attr('data-src'));
        }
    });*/

    function wakeupGraphics() {
        clearSlides();
        $.ajax({
            url: 'php/engine.php',
            type: 'POST',
            ...
            success: function (data, textStatus, jqXHR) {
                if (!data.error) {
                    getSlide(4);
                    setTimeout(function () {
                        graphicTwo(data.not, data.maybe, data.yes);
                        graphicOne(data.happy, data.bad);
                    }, 2300);
                } else {
                    alert(data.msg);
                }
            ...
            error: function (jqXHR, textStatus, errorThrown) {
                alert('Erro ao obter graficos' + jqXHR);
                //msgBox('Status: ' + jqXHR.status);
                //msgBox('Text error: ' + textStatus);
                //msgBox('More: ' + errorThrown);
                //msgBox('Response: ' + jqXHR.responseText);
    ...
    //Botão de chamada do slide de estatisticas
    ...
    /*$('.btnquestion').on('click', function () {
     sendButton($(this).attr('id'));
     recoveryAnime($(this).attr('data-id'), $(this).attr('data-src'));
     pushslide();
     });*/

    /*$('.tagbtnconfig').on('click', function () {
     pushslide();
     });*/
});
